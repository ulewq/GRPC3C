...
class CallMaker:
    def __init__(
        ...
        self._stub = stream_server_pb2_grpc.PhoneStub(self._channel)
    ...
    def _response_watcher(
        self, response_iterator: Iterator[stream_server_pb2.StreamingResponse]
    ) -> None:
        try:
    ...
    def _on_call_info(self, call_info: stream_server_pb2.CallInfo) -> None:
        self._session_id = call_info.session_id
        self._audio_session_link = call_info.media

    def _on_call_state(self, call_state: stream_server_pb2.CallState.State) -> None:
        logging.info(
            "Call toward [%s] enters [%s] state",
            self._phone_number,
            stream_server_pb2.CallState.State.Name(call_state),
        )
        self._call_state = call_state
        if call_state == stream_server_pb2.CallState.State.ACTIVE:
            self._peer_responded.set()
        if call_state == stream_server_pb2.CallState.State.ENDED:
            self._peer_responded.set()
            self._call_finished.set()

    def call(self) -> None:
        request = stream_server_pb2.StreamingRequest()
        request.phone_number = self._phone_number
        response_iterator = self._stub.Streaming(iter((request,)))
    ...
    def wait_peer(self) -> bool:
        ...
        return self._call_state == stream_server_pb2.CallState.State.ACTIVE
